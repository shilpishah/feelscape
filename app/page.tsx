  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-900">
      ...
      {!loading && !showLandscape && (
        <IntroScreen handleGetStarted={handleGetStarted} isHovered={isHovered} setIsHovered={setIsHovered} />
      )}

      {/* Landscape */}
      {showLandscape && (
        <div
          className="absolute inset-0 z-20 animate-fadeIn transition-all duration-1000"
          style={{
        ...
        >
          <LandscapeMenu
            ...
            setShowMoodPopup={setShowMoodPopup}
            setShowMusicPopup={setShowMusicPopup}
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: `
            @keyframes orbit1 {0%{transform:rotate(0deg) translateX(24px) rotate(0deg);}100%{transform:rotate(360deg) translateX(24px) rotate(-360deg);}}
            @keyframes orbit2 {0%{transform:rotate(120deg) translateX(24px) rotate(-120deg);}100%{transform:rotate(480deg) translateX(24px) rotate(-480deg);}}
            @keyframes orbit3 {0%{transform:rotate(240deg) translateX(24px) rotate(-240deg);}100%{transform:rotate(600deg) translateX(24px) rotate(-600deg);}}
            ...
            @keyframes float{0%,100%{transform:translateY(0px) translateX(0px);opacity:0.3;}25%{opacity:0.7;}50%{transform:translateY(-120px) translateX(30px);opacity:0.9;}75%{opacity:0.6;}}
            .animate-float{animation:float linear infinite;}
            @keyframes fadeIn{from{opacity:0;}to{opacity:1;}}.animate-fadeIn{animation:fadeIn 0.5s ease-in-out forwards;}
          `,
        }}
      />
    </div>
...
const IntroScreen: React.FC<{ handleGetStarted: () => void; isHovered: boolean; setIsHovered: (h: boolean) => void }> = ({ handleGetStarted, isHovered, setIsHovered }) => (
...
const LandscapeMenu: React.FC<{
  menuOpen: boolean;
  setMenuOpen: (open: boolean) => void;
  setShowMoodPopup: (b: boolean) => void;
  setShowMusicPopup: (b: boolean) => void;
  setShowHelpPopup: (b: boolean) => void;
}> = ({ menuOpen, setMenuOpen, setShowMoodPopup, setShowMusicPopup, setShowHelpPopup }) => (
  <div className="absolute top-4 right-4 z-50 flex flex-col items-end space-y-3">
    ...
    <div
      className={`flex flex-col items-center space-y-3 transition-transform transition-opacity duration-300 ease-out origin-top-right ${
        menuOpen ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-95 -translate-y-4 pointer-events-none"
      }`}
    >
  ...
  return (
    <div className="relative group">
      <button
        ...
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className="p-3 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 hover:bg-white/30 hover:scale-110 transition-all duration-300 hover:shadow-lg hover:shadow-white/25"
      >
        <Icon className="w-5 h-5 text-white" />
      </button>
      <div
        className={`absolute top-1/2 right-full mr-3 transform -translate-y-1/2 bg-black/80 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-lg whitespace-nowrap transition-all duration-200 ${
